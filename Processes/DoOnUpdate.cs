...
namespace Processes
{
    ...
    //Setting iterationsLeft to -1 means the action will be executed infitely until the condition is met as lon as it is not null
    ...
    public sealed class DoOnUpdate : Process
    {
        ...
        public override bool Invoke()
        {
            ...
            {
                if (condition != null && condition()) //If the condition is met, perform the action if it is not null, then interrupt the process chain
                {
                    if (action != null && !action.Equals(null))
                        action();
                    return false;
